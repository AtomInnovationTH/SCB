...
> - **#12 flags:** `isFlagEligible` is SIZE-gated (any object ≥ `FLAG_MIN_SIZE_M` 2 m, excluding small `missionDebris`) so large derelict sats the catalog tags "debris" (COSMOS 1408/2251, IRIDIUM 33, FENGYUN-1C) get flags too — not just rocketBody/defunctSat; sub-threshold fragments still never do. Plus `pickCountryForId` (deterministic weighted, procedural sats/rockets get countries at build); decal sized in physical metres `clamp(sizeMeter × 0.12, 0.4..1.6)` preserving LOD ratio.

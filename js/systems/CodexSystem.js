...
function buildEntries() {
  return [
    ...
    },
    {
      ...
      fullText: 'Inclination determines which latitudes a satellite passes over. The ISS orbits at 51.6°, seeing most populated areas. Changing inclination requires burning perpendicular to your orbit — extremely expensive in fuel. This is why debris in different inclinations must be cleared in separate passes rather than chased across planes.',
      triggerEvent: Events.TARGET_SELECTED,
    ...
    // === PROPULSION (6) ===
    {
      ...
      fullText: 'FEEP thrusters ionize liquid metal (typically indium or cesium) and accelerate ions through electric fields. They produce tiny thrust (micro-Newtons) but incredible efficiency (Isp >6000s). Your daughters use miniaturized FEEP units — perfect for precise maneuvering in the debris field.',
    ...
    },
    {
      ...
      fullText: 'A typical S-band downlink carries ~2 Mbps — less than a poor WiFi connection. With 6 active daughters generating telemetry, sensor data, and video, bandwidth becomes a real constraint. Data compression, prioritization, and store-and-forward help, but sometimes you have to choose which daughter gets real-time data.',
      triggerEvent: Events.COMMS_MESSAGE,
    ...
    },
    {
      ...
      fullText: 'Rendezvous and proximity operations (RPO) use a combination of LIDAR, optical cameras, and relative GPS to achieve centimeter precision. Your daughter\'s final approach to debris uses similar techniques: closing velocity must drop below 0.1 m/s, alignment within 5°. One wrong move and you bounce the target into an unpredictable tumble.',
      triggerEvent: Events.COMMS_MESSAGE,
    ...
    },
    {
      ...
      fullText: 'Defunct satellites and debris tumble unpredictably at rates up to 60°/s. Before hauling, you must despin the target. Your daughter applies counter-torque through its grapple point, using thrusters or reaction wheels. The challenge: asymmetric objects have complex rotation (all three axes). Understanding moment of inertia helps predict how much effort despinning takes.',
      triggerEvent: Events.COMMS_MESSAGE,
    ...
    },
    {
      ...
      category: CodexCategory.POWER,
      shortText: 'Burst power devices that charge in seconds — perfect for deploying daughters and lasso.',
      fullText: 'Supercapacitors store less total energy than batteries but can discharge it 100× faster. They\'re perfect for brief high-power operations: deploying a daughter, launching a lasso, activating electromagnets. Your spacecraft charges supercap banks from solar panels, then dumps the energy in milliseconds when needed. They last millions of cycles with no degradation.',
      triggerEvent: Events.COMMS_MESSAGE,
    ...
    },
    {
      ...
      fullText: 'In LEO, a 10 km altitude difference produces ~5 m/s relative drift — walking speed. Objects in the exact same orbit appear stationary to each other, even though both travel at 7.8 km/s. This is why rendezvous is about matching orbits, not chasing targets. Your daughters exploit this: launch them into matched orbits and they drift gently toward debris.',
      triggerEvent: Events.ARM_CAPTURED,
      triggerCondition: () => true,
      unlocked: false,
      seen: false,
    ...
    },
    {
      ...
      triggerEvent: Events.AUTOPILOT_ENGAGE,
      triggerCondition: () => true,
      unlocked: false,
      seen: false,
      icon: '⏱️',
    ...
    // === PROPULSION (new — 4 entries) ===
    {
      ...
      category: CodexCategory.PROPULSION,
      shortText: 'Cradle daughters use compressed springs — stored mechanical energy releases with zero propellant cost.',
      fullText: 'Spring-launched daughters use stored elastic potential energy (½kx²). Your cradle compresses a spring via worm gear, storing 2–5 J of energy. On release, this accelerates the 2–7 kg daughter to 0.5–1.5 m/s — enough for LEO proximity operations. Zero propellant cost for the launch itself, though FEEP thrusters handle final approach.',
    ...
    },
    {
      ...
      fullText: 'Tether tangles are the bane of multi-daughter operations. When two tethers cross under tension, friction holds them locked. The solution: inject a precisely timed slack pulse into one tether, allowing it to float free while the other remains taut. Your cradle system detects tangles via tension sensors and can auto-resolve minor crossings, but severe tangles require manual intervention — or cutting free.',
    ...
    },
    {
      ...
      fullText: 'Unlike traditional spacecraft capture systems that use propulsion for retrieval, your daughters use a motorized reel on the mothership. At 15 W power draw, the reel winches tether at 0.25 m/s loaded (0.5 m/s unloaded). The mechanical advantage means zero propellant cost for retrieval — only electrical power. A brake system handles tension spikes during retrieval of tumbling objects.',
      triggerEvent: Events.TETHER_REEL_STATE,
      triggerCondition: (p) => p.reeling === true,
      unlocked: false,
      seen: false,
    ...
    },
    {
      ...
      category: CodexCategory.SENSORS,
      shortText: 'Multiple daughters transmit radar pulses simultaneously — distributed aperture gives superior resolution.',
      fullText: 'A single small radar antenna has poor angular resolution. But by timing pulses from multiple daughters spread across hundreds of meters, your cradle system creates a synthetic aperture — a virtual antenna the size of the entire daughter constellation. This distributed pulse scan can detect debris down to 1 cm at ranges your single mothership radar could never achieve.',
    ...
    },
    {
      ...
      fullText: 'LIDAR (Light Detection and Ranging) fires laser pulses and measures the return time to calculate distance. At close range (<1 km), it provides millimeter-precision distance and centimeter-resolution 3D mapping of debris surfaces. Your daughters use LIDAR during the APPROACH state to build a tumble model of the target before committing to net deployment.',
      triggerEvent: Events.ARM_CAPTURED,
      triggerCondition: () => true,
      unlocked: false,
      seen: false,
    ...
    },
    {
      ...
      fullText: 'Standard spacecraft communication uses S-band (~2 GHz) providing 1-2 Mbps. With multiple daughters transmitting telemetry simultaneously, bandwidth becomes a constraint — you can\'t get high-resolution video from all daughters at once. Ka-band (~26 GHz) offers 10× higher data rates but requires more precise antenna pointing. Laser comms (optical) can reach 100+ Mbps.',
      triggerEvent: Events.SUBSYSTEM_EVENT,
      triggerCondition: (p) => p.text && p.text.toLowerCase().includes('bandwidth'),
      unlocked: false,
      seen: false,
    ...
    },
    {
      ...
      fullText: 'Laser communication uses modulated infrared beams (1550 nm) instead of radio waves. NASA\'s LCRD demonstrated 1.2 Gbps from geostationary orbit in 2021. Advantages: 10-100× higher data rates, smaller terminals, no frequency licensing. Challenges: requires precise pointing (50 µrad beam), blocked by clouds, and atmospheric turbulence causes scintillation. Your optical comms upgrade enables real-time video from daughters.',
      triggerEvent: Events.SUBSYSTEM_EVENT,
      triggerCondition: (p) => p.text && p.text.toLowerCase().includes('laser comms'),
      unlocked: false,
      seen: false,
    ...
    },
    {
      ...
      fullText: 'Radio signals travel at the speed of light — 300,000 km/s. At LEO altitude (400 km), the raw propagation delay is just 1.3 ms. But real-world latency includes encoding, error correction, relay satellite hops (if using TDRS: up to GEO and back = ~240 ms), ground processing, and network routing. Total round-trip latency: 400-800 ms — noticeable during manual daughter piloting.',
      triggerEvent: Events.COMMS_MESSAGE,
    ...
    },
    {
      ...
      fullText: 'Spacecraft electrical power is distributed through regulated buses (typically 28V or 100V DC). When power generation drops (eclipse, panel failure), an Energy Transfer System (ETS) sheds non-critical loads in priority order: science instruments first, then heaters, then communications, with attitude control and life support last to go. Your three-bus power system (Thrust/Sensors/Daughters) lets you manually prioritize.',
...
const UNLOCK_HINTS = {
  ...
  orbital_inclination: 'Target debris (T), then engage autopilot (A).',
